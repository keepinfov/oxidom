...
impl Service {
    ...
    /// the session is `default`, `default.toml` is read for the settings a
    /// session needs — its interface, its `[core]`, its routing block and what
    /// it does when a core exits — and **no profile file is written or
    /// modified**.
