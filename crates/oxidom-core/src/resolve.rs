...
mod tests {
    ...
    fn runtime_info_json_shape_is_unchanged() {
        ...
        assert_eq!(
            ...
            r#"{"xray_path":"/nix/store/xray/bin/xray","xray_error":null,"xray_source":"config","socks_port_locked":false,"http_port_locked":false,"binary_paths_locked":false,"socks_port":0,"http_port":0,"geo":{"dir":"","usable":null,"error":null,"geoip_bytes":0,"geosite_bytes":0,"writable":false,"downloading":false,"current_file":null,"done_bytes":0,"total_bytes":0,"last_error":null,"cancelled":false},"daemon_version":null,"core_version":null}"#
        );
    }
    ...
    /// And once more for the versions, which arrived last. A daemon that
    /// predates them says nothing, and `None` must mean "this daemon is too
    /// old to say" rather than "there is no core" — the second is what
    /// `core_version` means when the daemon *is* new enough, and conflating
    /// the two would have every old daemon accused of having no core.
    #[test]
    fn runtime_info_without_the_versions_still_parses() {
        let older = r#"{"xray_path":"/usr/bin/xray","socks_port_locked":true,"socks_port":1080}"#;
        let info: crate::ipc::RuntimeInfo = serde_json::from_str(older).unwrap();
        assert_eq!(info.daemon_version, None);
        assert_eq!(info.core_version, None);
        assert_eq!(info.xray_path.as_deref(), Some("/usr/bin/xray"));
    }
