...
/// The one path a stored server becomes an Xray outbound through; `draft`
/// proves a hand-entered server against it before anything is created.
pub(crate) fn outbound_tagged(server: &Server, tag: &str) -> Option<Value> {
    let addr = &server.address;
    let port = server.port;
    let mut value = match &server.spec {
        OutboundSpec::Vless {
            uuid,
            encryption,
            stream,
...
/// RFC 7396 merge: objects merge key by key, `null` removes a key, anything
/// else replaces. What lets a raw fragment adjust the generated outbound
/// without the generator modelling every core option.
pub(crate) fn merge_patch(base: &mut Value, patch: &Value) {
    match patch {
        Value::Object(patch) => {
            ...
            let slots = base.as_object_mut().expect("just made an object");
