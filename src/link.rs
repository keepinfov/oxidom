...
use crate::model::{
    country_from_name, transport_label, OutboundSpec, Protocol, Server, StreamSettings,
};
...
fn stream_from_query(q: &HashMap<String, String>) -> StreamSettings {
    ...
    StreamSettings {
        ...
        fingerprint: get("fp"),
        allow_insecure: q.get("allowInsecure").map(|v| v == "1" || v == "true").unwrap_or(false),
        public_key: get("pbk"),
        short_id: get("sid"),
        spider_x: get("spx"),
        path: get("path"),
...
fn parse_vless(link: &str) -> Option<Server> {
    ...
    let uuid = percent_decode_str(url.username()).decode_utf8_lossy().into_owned();
    if uuid.is_empty() {
        return None;
    }
    ...
    let encryption = q.get("encryption").cloned().unwrap_or_else(|| "none".to_string());
    let spec = OutboundSpec::Vless { uuid, encryption, stream: stream.clone() };
    Some(finish(link, name, Protocol::Vless, host, port, Some(&stream), spec))
}
...
fn parse_trojan(link: &str) -> Option<Server> {
    ...
    let spec = OutboundSpec::Trojan { password, stream: stream.clone() };
    Some(finish(link, name, Protocol::Trojan, host, port, Some(&stream), spec))
}
...
fn parse_vmess(link: &str) -> Option<Server> {
    ...
    let num = |k: &str| -> Option<u64> {
        json.get(k).and_then(|v| v.as_u64().or_else(|| v.as_str().and_then(|s| s.parse().ok())))
    };
    ...
    let stream = StreamSettings {
        network: net,
        security: if tls.is_empty() { "none".to_string() } else { tls },
    ...
    let spec = OutboundSpec::Vmess { uuid, alter_id, security, stream: stream.clone() };
    Some(finish(link, name, Protocol::Vmess, host, port, Some(&stream), spec))
}
...
fn parse_ss(link: &str) -> Option<Server> {
    ...
    Some(finish(link, name, Protocol::Shadowsocks, host, port, None, spec))
}
