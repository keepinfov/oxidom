...
- `vless://uuid@host:port?<params>#name` — params: `type` (tcp/ws/grpc/xhttp/splithttp),
  ...
  `path`, `host`, `serviceName`, `authority`, `mode`, `alpn`, `encryption`. XHTTP preserves its
  `mode` (`auto`, `packet-up`, `stream-up`, `stream-one`); gRPC preserves `authority` and
  `mode=multi`, while a leading slash in `serviceName` is removed only when generating Xray JSON.
